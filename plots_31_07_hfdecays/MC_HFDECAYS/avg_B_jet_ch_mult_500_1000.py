...
ax_yScale = 'linear'
xLims = (0.5, 40.5)
yLims = (-0.055062238000000006, 0.31425405)
xMinorTickMarks = 0

# TeX-friendly labels for the legend
labels = {
    'legend' : ([ r"Recoil to top", r"Recoil to b" ], r""),
}
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['recoil_t'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['recoil_t']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['recoil_t'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['recoil_t'],
