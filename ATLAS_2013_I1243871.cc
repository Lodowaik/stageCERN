//questa è la versione modificata, cioè con gli istogrammi aggiuntivi per pt alti
...
namespace Rivet {
  ...
  public:
    ...
    book(_p_b_rho[d],
         "b_rho_ptbin" + to_str(d+1),
         10, 0.0, 0.4);
     book(_p_l_rho[d],
         "l_rho_ptbin" + to_str(d+1),
         10, 0.0, 0.4);
      book(_p_b_Psi[d],
         "b_Psi_ptbin" + to_str(d+1),
         10, 0.0, 0.4);
       book(_p_l_Psi[d],
         "l_Psi_ptbin" + to_str(d+1),
         10, 0.0, 0.4);
}
}
}
    ...
    void analyze(const Event& event) {
      ...
      vector<double> ptEdges; ptEdges += {{ 30, 40, 50, 70, 100, 150, 200, 300 }};
      ...
      for (const Jet* bJet : b_jets) {
        ...
        size_t ipt; for (ipt = 0; ipt < 7; ++ipt) if (inRange(jetPt/GeV, ptEdges[ipt], ptEdges[ipt+1])) break;
  ...
  private:
    ...
    Profile1DPtr _p_b_rho[7];
    Profile1DPtr _p_l_rho[7];
    Profile1DPtr _p_b_Psi[7];
    Profile1DPtr _p_l_Psi[7];
