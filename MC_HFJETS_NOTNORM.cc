...
namespace Rivet {
  ...
  class MC_HFJETS_NOTNORM : public Analysis {
  public:

    // Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(MC_HFJETS_NOTNORM);
    ...
    void analyze(const Event& event) {
      ...
      // questa riga qui sotto la commento perché secondo chatgpt con il fatto che non applica una richiesta sul pt minimo del jet è probbabilemnete ciò che mi manda in overflow la baracca
      //const Jets& jets = apply<JetFinder>(event, "Jets").jetsByPt();
      //questa riga qui sotto è quello che lo sostituisce:
      ...
      for (const Jet& j : jets) {
        ...
        if (!gotLeadingB && j.bTagged(Cuts::pT > 500*MeV)) {
         ...
         //qui comincia un debugging suggeritomi da chatgpt per capire se il fatto che qui ci siano dei valori in overflow è ciò che poi mi impedisce di normalizzare correttamente gli istogrammi di rtt
	 double frac = bhad.E() / j.E();

     if (!std::isfinite(frac))
        MSG_WARNING("Bad fraction!");

      if (frac > 2 || frac < 0)
        MSG_WARNING("frac = " << frac
              << "  EB=" << bhad.E()/GeV
              << "  EJ=" << j.E()/GeV);

        _h_eFracB->fill(frac);
           //qui finisce il codice suggerito da chatgpt
	//  _h_eFracB->fill(bhad.E() / j.E()); //questa riga la commento perché fa la stessa cosa che fa la riga subito sopra
          continue;
        }
        // c-tag testing
    ...
    void finalize() {
      //normalize({_h_ptCJetLead, _h_ptCHadrLead, _h_ptBJetLead, _h_ptBHadrLead,
       ...
       MSG_INFO("=== FINALIZE MC_HFJETS_NOTNORM ===");

        MSG_INFO("Entries ptBJetLead = "
			           << _h_ptBJetLead->numEntries());
  ...
  RIVET_DECLARE_PLUGIN(MC_HFJETS_NOTNORM);
