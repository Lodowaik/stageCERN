...
# Analysis object: /MC_HFDECAYS/lepton_pTrel_C_jets
...
ax_title  = r'$p_\mathrm{T,rel}$ distribution for leptons in $C$-jets'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 10.0)
yLims = (9e-05, 1.1)
...
exec(open(prefix+'lepton_pTrel_C_jets__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'lepton_pTrel_C_jets.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'lepton_pTrel_C_jets.png'), format='PNG')
