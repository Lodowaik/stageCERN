...
# Analysis object: /MC_HFDECAYS/B0_charged_mult
...
ax_title  = r'$N_\mathrm{ch}$ of $B^0$ charged stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 30.5)
yLims = (0.0, 0.44876612)
...
exec(open(prefix+'B0_charged_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'B0_charged_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'B0_charged_mult.png'), format='PNG')
