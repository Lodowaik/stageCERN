...
# Analysis object: /MC_HFDECAYS/pT_B_jet
...
ax_xLabel = r'$p_\mathrm{T}^\mathrm{jet}$ [GeV]'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}p_\mathrm{T}$'
ax_title  = r'$p_\mathrm{T}^\mathrm{jet}$ distribution for $B$-jets'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (25.0, 1025.0)
yLims = (9e-08, 0.11000000000000001)
...
exec(open(prefix+'pT_B_jet__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'pT_B_jet.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'pT_B_jet.png'), format='PNG')
