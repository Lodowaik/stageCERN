...
# Analysis object: /MC_HFDECAYS/lepton_pT_B_jets
...
ax_title  = r'$p_\mathrm{T}$ distribution for leptons in $B$-jets'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 100.0)
...
exec(open(prefix+'lepton_pT_B_jets__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'lepton_pT_B_jets.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'lepton_pT_B_jets.png'), format='PNG')
