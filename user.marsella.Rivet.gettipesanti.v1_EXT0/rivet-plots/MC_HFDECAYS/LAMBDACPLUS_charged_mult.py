...
# Analysis object: /MC_HFDECAYS/LAMBDACPLUS_charged_mult
...
ax_xLabel = r'$N_\mathrm{ch}$ of charged decay products'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}N_\mathrm{ch}$'
ax_title  = r'$N_\mathrm{ch}$ of $\Lambda^+_c$ charged stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 16.5)
yLims = (0.0, 0.58096709)
...
exec(open(prefix+'LAMBDACPLUS_charged_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'LAMBDACPLUS_charged_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'LAMBDACPLUS_charged_mult.png'), format='PNG')
