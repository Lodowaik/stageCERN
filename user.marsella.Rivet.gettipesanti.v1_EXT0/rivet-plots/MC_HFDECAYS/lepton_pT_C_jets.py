...
# Analysis object: /MC_HFDECAYS/lepton_pT_C_jets
...
ax_xLabel = r'$p_\mathrm{T}$ [GeV]'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}p_\mathrm{T}$'
ax_title  = r'$p_\mathrm{T}$ distribution for leptons in $C$-jets'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 100.0)
yLims = (9e-06, 0.11000000000000001)
...
exec(open(prefix+'lepton_pT_C_jets__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'lepton_pT_C_jets.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'lepton_pT_C_jets.png'), format='PNG')
