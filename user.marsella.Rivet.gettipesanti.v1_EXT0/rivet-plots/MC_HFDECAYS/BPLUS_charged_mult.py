...
# Analysis object: /MC_HFDECAYS/BPLUS_charged_mult
...
ax_title  = r'$N_\mathrm{ch}$ of $B^+$ charged stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 30.5)
yLims = (0.0, 0.41941174000000003)
...
exec(open(prefix+'BPLUS_charged_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'BPLUS_charged_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'BPLUS_charged_mult.png'), format='PNG')
