...
# Analysis object: /MC_HFDECAYS/avg_B_jet_l_pTrel_500_1000
# Timestamp: 24-07-2026 (16:40:05)
...
ax_xLabel = r'$p_\mathrm{T,rel}$[GeV]'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}p_\mathrm{T}$'
ax_title  = r'$p_\mathrm{T,rel}$ lepton $B$-jets 500 GeV$<p_\mathrm{T}<$1000 GeV'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 15.0)
yLims = (0.009000000000000001, 1.1)

# TeX-friendly labels for the legend
labels = {
    'legend' : ([ r"user.marsella.51666151.EXT0.\_000002.gettipesanti" ], r""),
}
...
exec(open(prefix+'avg_B_jet_l_pTrel_500_1000__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['user.marsella.51666151.EXT0._000002.gettipesanti.yoda.gz'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['user.marsella.51666151.EXT0._000002.gettipesanti.yoda.gz']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['user.marsella.51666151.EXT0._000002.gettipesanti.yoda.gz'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['user.marsella.51666151.EXT0._000002.gettipesanti.yoda.gz'],
...
plt.savefig(os.path.join(plot_outdir, 'avg_B_jet_l_pTrel_500_1000.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'avg_B_jet_l_pTrel_500_1000.png'), format='PNG')
