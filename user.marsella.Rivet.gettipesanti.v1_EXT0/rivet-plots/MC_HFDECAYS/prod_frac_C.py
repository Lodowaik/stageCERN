...
# Analysis object: /MC_HFDECAYS/prod_frac_C
...
ax_xLabel = r''
ax_yLabel = r'Production fraction'
ax_title  = r'Production fractions for various weakly decaying c-hadrons'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 7.5)
yLims = (0.0, 0.62034511)
xMajorTickMarks = []
xCustomTickLabels = []
...
exec(open(prefix+'prod_frac_C__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'prod_frac_C.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'prod_frac_C.png'), format='PNG')
