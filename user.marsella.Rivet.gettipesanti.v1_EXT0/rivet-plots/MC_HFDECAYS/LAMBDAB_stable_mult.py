...
# Analysis object: /MC_HFDECAYS/LAMBDAB_stable_mult
...
ax_title  = r'$N_\mathrm{st}$ of $\Lambda^0_b$ stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 40.5)
yLims = (0.0, 0.13518538000000002)
...
exec(open(prefix+'LAMBDAB_stable_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'LAMBDAB_stable_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'LAMBDAB_stable_mult.png'), format='PNG')
