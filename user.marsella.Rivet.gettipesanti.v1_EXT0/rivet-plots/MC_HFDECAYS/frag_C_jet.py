...
# Analysis object: /MC_HFDECAYS/frag_C_jet
...
ax_xLabel = r'z $\equiv$ $p_\mathrm{jet}\cdot p_C/|p_\mathrm{jet}|^2$'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}z$'
ax_title  = r'Fragmentation function for $C$-jets'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.0, 1.1)
yLims = (0.0, 2.1821283000000005)
...
exec(open(prefix+'frag_C_jet__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'frag_C_jet.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'frag_C_jet.png'), format='PNG')
