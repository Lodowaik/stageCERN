...
# Analysis object: /MC_HFDECAYS/B0S_charged_mult
...
ax_title  = r'$N_\mathrm{ch}$ of $B^0_s$ charged stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 30.5)
yLims = (0.0, 0.4279471900000001)
...
exec(open(prefix+'B0S_charged_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'B0S_charged_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'B0S_charged_mult.png'), format='PNG')
