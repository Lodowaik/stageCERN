...
# Analysis object: /MC_HFDECAYS/LAMBDACPLUS_stable_mult
...
ax_xLabel = r'$N_\mathrm{st}$ of stable decay products'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}N_\mathrm{st}$'
ax_title  = r'$N_\mathrm{st}$ of $\Lambda^+_c$ stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 20.5)
yLims = (0.0, 0.31002818)
...
exec(open(prefix+'LAMBDACPLUS_stable_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'LAMBDACPLUS_stable_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'LAMBDACPLUS_stable_mult.png'), format='PNG')
