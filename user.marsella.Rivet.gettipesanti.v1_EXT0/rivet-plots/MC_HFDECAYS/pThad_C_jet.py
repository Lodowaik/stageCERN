...
# Analysis object: /MC_HFDECAYS/pThad_C_jet
...
ax_xLabel = r'$p_\mathrm{T}^\mathrm{hadron}$ [GeV]'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}p_\mathrm{T}$'
ax_title  = r'$p_\mathrm{T}^\mathrm{hadron}$ distribution for $C$-jets'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 200.0)
...
exec(open(prefix+'pThad_C_jet__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'pThad_C_jet.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'pThad_C_jet.png'), format='PNG')
