...
# Analysis object: /MC_HFDECAYS/pThad_B_jet
...
ax_title  = r'$p_\mathrm{T}^\mathrm{hadron}$ distribution for $B$-jets'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (0.0, 200.0)
yLims = (9e-05, 0.11000000000000001)
...
exec(open(prefix+'pThad_B_jet__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'pThad_B_jet.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'pThad_B_jet.png'), format='PNG')
