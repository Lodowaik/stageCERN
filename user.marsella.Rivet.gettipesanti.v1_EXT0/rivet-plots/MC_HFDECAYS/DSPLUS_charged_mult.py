...
# Analysis object: /MC_HFDECAYS/DSPLUS_charged_mult
...
ax_title  = r'$N_\mathrm{ch}$ of $D^+_s$ charged stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 16.5)
yLims = (0.0, 0.5146713)
...
exec(open(prefix+'DSPLUS_charged_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'DSPLUS_charged_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'DSPLUS_charged_mult.png'), format='PNG')
