...
# Analysis object: /MC_HFDECAYS/frag_B_jet
...
ax_xLabel = r'z $\equiv$ $p_\mathrm{jet}\cdot p_B/|p_\mathrm{jet}|^2$'
ax_yLabel = r'$1/\sigma \mathrm{d}\sigma/\mathrm{d}z$'
ax_title  = r'Fragmentation function for $B$-jets'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.0, 1.1)
yLims = (0.0, 3.6796155000000006)
...
exec(open(prefix+'frag_B_jet__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'frag_B_jet.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'frag_B_jet.png'), format='PNG')
