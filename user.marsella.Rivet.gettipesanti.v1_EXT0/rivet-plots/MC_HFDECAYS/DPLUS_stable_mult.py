...
# Analysis object: /MC_HFDECAYS/DPLUS_stable_mult
...
ax_title  = r'$N_\mathrm{st}$ of $D^+$ stable decay products'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.5, 20.5)
yLims = (0.0, 0.34287484)
...
exec(open(prefix+'DPLUS_stable_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'DPLUS_stable_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'DPLUS_stable_mult.png'), format='PNG')
