...
# Analysis object: /MC_HFJETS_NOTNORM/ptCJetLead
# Timestamp: 29-07-2026 (18:13:05)
...
ax_yScale = 'log'
xLims = (0.0, 200.0)
yLims = (900.0, 110000.00000000001)

# TeX-friendly labels for the legend
labels = {
    'legend' : ([ r"hfjets\_rtb\_normalized\_124" ], r""),
}
...
exec(open(prefix+'ptCJetLead__data.py').read(), dataf)
...
styles = {
  'hfjets_rtb_normalized_124.yoda.gz': {
    'color' : '#EE3311',
    'linestyle' : '-',
    'linewidth' : 1,
    'marker' : 'none',
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['hfjets_rtb_normalized_124.yoda.gz'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['hfjets_rtb_normalized_124.yoda.gz']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['hfjets_rtb_normalized_124.yoda.gz'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['hfjets_rtb_normalized_124.yoda.gz'],
...
plt.savefig(os.path.join(plot_outdir, 'ptCJetLead.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'ptCJetLead.png'), format='PNG')
