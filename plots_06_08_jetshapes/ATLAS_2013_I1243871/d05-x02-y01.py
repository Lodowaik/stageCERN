...
# Analysis object: /ATLAS_2013_I1243871/d05-x02-y01
# Timestamp: 06-08-2026 (15:14:31)
...
ax_xLabel = r'$r$'
ax_yLabel = r'$\rho(r)$'
ax_title  = r'Differential jet shape for light-jets with $100\,\mathrm{GeV} < p_T < 150\,\mathrm{GeV}$'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.0, 0.4)
yLims = (0.0, 16.941155000000002)
...
exec(open(prefix+'d05-x02-y01__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['recoil_b'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['recoil_b']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['recoil_b'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['recoil_b'],
...
plt.savefig(os.path.join(plot_outdir, 'd05-x02-y01.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'd05-x02-y01.png'), format='PNG')
