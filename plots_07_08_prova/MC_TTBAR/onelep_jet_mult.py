...
# Analysis object: /MC_TTBAR/onelep_jet_mult
...
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}N_\mathrm{jets}$ [pb]'
ax_title  = r'Jet multiplicity'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (-0.5, 10.5)
yLims = (0.0, 0.12208636000000002)
...
exec(open(prefix+'onelep_jet_mult__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'onelep_jet_mult.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'onelep_jet_mult.png'), format='PNG')
