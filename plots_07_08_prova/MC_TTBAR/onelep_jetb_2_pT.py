...
# Analysis object: /MC_TTBAR/onelep_jetb_2_pT
...
ax_title  = r'Transverse momentum distribution for $b$-jet 2'
ax_xScale = 'log'
ax_yScale = 'log'
xLims = (20.0, 300.0)
...
exec(open(prefix+'onelep_jetb_2_pT__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'onelep_jetb_2_pT.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'onelep_jetb_2_pT.png'), format='PNG')
