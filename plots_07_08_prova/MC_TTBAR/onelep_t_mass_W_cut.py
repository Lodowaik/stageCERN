...
# Analysis object: /MC_TTBAR/onelep_t_mass_W_cut
# Timestamp: 07-08-2026 (10:19:45)
...
ax_xLabel = r'$m$ [GeV]'
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}m$ [pb GeV$^{-1}$]'
ax_title  = r'Mass distribution for reconstructed top after $m_W$ cut'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (130.0, 430.0)
yLims = (0.0, 0.0020551949)

# TeX-friendly labels for the legend
labels = {
    'legend' : ([ r"mod1\_ttbar\_nonallhad\_rtb\_evnt.124" ], r""),
}
...
exec(open(prefix+'onelep_t_mass_W_cut__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['mod1_ttbar_nonallhad_rtb_evnt.124.yoda.gz'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['mod1_ttbar_nonallhad_rtb_evnt.124.yoda.gz']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['mod1_ttbar_nonallhad_rtb_evnt.124.yoda.gz'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['mod1_ttbar_nonallhad_rtb_evnt.124.yoda.gz'],
...
plt.savefig(os.path.join(plot_outdir, 'onelep_t_mass_W_cut.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'onelep_t_mass_W_cut.png'), format='PNG')
