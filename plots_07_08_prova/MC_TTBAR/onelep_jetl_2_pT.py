...
# Analysis object: /MC_TTBAR/onelep_jetl_2_pT
...
ax_yLabel = r'$\mathrm{d}\sigma/\mathrm{d}p_\perp$ [pb GeV$^{-1}$]'
ax_title  = r'Transverse momentum distribution for light jet 2'
ax_xScale = 'log'
ax_yScale = 'log'
xLims = (20.0, 300.0)
yLims = (9e-07, 0.011000000000000001)
...
exec(open(prefix+'onelep_jetl_2_pT__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'onelep_jetl_2_pT.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'onelep_jetl_2_pT.png'), format='PNG')
