...
ax_yScale = 'log'
xLims = (0.0, 200.0)
yLims = (900000.0, 110000000.00000001)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['YYY'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['YYY']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['YYY'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['YYY'],
