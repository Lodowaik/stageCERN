...
labels = {
    'legend' : ([ r"Recoil to b", r"Recoil to top" ], r""),
}
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['rec_b'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['rec_b']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['rec_b'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['rec_b'],
